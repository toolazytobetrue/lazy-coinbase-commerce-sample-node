...
export async function transactionCreateGoldOrder(currency: string, goldType: string, units: number, stock: StockDocument, paymentGateway: PaymentGatewayDocument, rsn: string, coupon: null | undefined | CouponDocument, ipAddress: string, userId?: string) {
    try {
        ...
        const price = +round(_price * RATES_MINIFIED[currency], 2)
        ...
        switch (paymentGateway.name) {
            case 'crypto':
                const coinbaseCharge = await createCoinbaseInvoice(currency, uuid, totalDiscounted, `${uuid}`, `Discount: ${coupon ? coupon.amount : 0}% - ${units}M GP ${goldType} - RSN: ${rsn}`);
                _order.payment = {
                    coinbase: {
