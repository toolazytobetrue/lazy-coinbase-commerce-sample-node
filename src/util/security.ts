...
export async function isAuthorized(req: Request, res: Response, next: NextFunction) {
    ...
    verifyDateInternal(token, [USER_PERMISSIONS.ADMIN, USER_PERMISSIONS.WORKER, USER_PERMISSIONS.CUSTOMER])
        .then(resolve => {
            return next();
        }).catch(err => {
...
export async function isAuthorizedBelowAdmin(req: Request, res: Response, next: NextFunction) {
    ...
    verifyDateInternal(token, [USER_PERMISSIONS.ADMIN, USER_PERMISSIONS.WORKER]).then(resolve => {
        return next();
    }).catch(err => {
        return res.status(401).send(err);
    });
